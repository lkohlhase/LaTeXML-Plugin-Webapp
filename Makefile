# This Makefile is for the LaTeXML::Plugin::Webapp extension to perl.
#
...
#     ABSTRACT => q[LaTeXML::Plugin::Webapp - A LaTeXML post-processor for generating webabbs]
#     AUTHOR => q[Lukas Kohlhase <kohliderzweite@gmail.com>]
#     BUILD_REQUIRES => {  }
#     META_MERGE => { resources=>{ repository=>{ web=>q[https://github.com/KWARC/LaTeXML-Plugin-Webapp], url=>q[https://github.com/KWARC/LaTeXML-Plugin-Webapp.git], type=>q[git] } }, meta-spec=>{ version=>q[2] } }
#     NAME => q[LaTeXML::Plugin::Webapp]
#     PREREQ_PM => { LaTeXML=>q[0.7.99] }
#     VERSION => q[0.1]
#     test => { TESTS=>q[t/*.t] }
...
NAME_SYM = LaTeXML_Plugin_Webapp
VERSION = 0.1
...
MAKE = make
FULLEXT = LaTeXML/Plugin/Webapp
BASEEXT = Webapp
PARENT_NAME = LaTeXML::Plugin
DLBASE = $(BASEEXT)
VERSION_FROM = 
OBJECT = 
LDFROM = $(OBJECT)
...
INST_LIBDIR      = $(INST_LIB)/LaTeXML/Plugin
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/LaTeXML/Plugin
...
DISTNAME = LaTeXML-Plugin-Webapp
DISTVNAME = LaTeXML-Plugin-Webapp-0.1
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               LaTeXML-Plugin-Webapp' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            0.1' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           LaTeXML::Plugin::Webapp - A LaTeXML post-processor for generating webabbs' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Lukas Kohlhase <kohliderzweite@gmail.com>' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    LaTeXML:  0.7.99' >> META_new.yml
	$(NOECHO) $(ECHO) 'resources:' >> META_new.yml
	$(NOECHO) $(ECHO) '    repository:' >> META_new.yml
	$(NOECHO) $(ECHO) '        type:  git' >> META_new.yml
	$(NOECHO) $(ECHO) '        url:   https://github.com/KWARC/LaTeXML-Plugin-Webapp.git' >> META_new.yml
	$(NOECHO) $(ECHO) '        web:   https://github.com/KWARC/LaTeXML-Plugin-Webapp' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    version:  2' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>LaTeXML::Plugin::Webapp - A LaTeXML post-processor for generating webabbs</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Lukas Kohlhase &lt;kohliderzweite@gmail.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="LaTeXML::" VERSION="0.7" />' >> $(DISTNAME).ppd
